    ...
    public partial class frmMain : Form
    {
        ...
        private void SaveROIBatch(bool xNoSave, Rectangle ImageDimensions)
        {
            ...
            redROI01.Parameters.Units = ViDi2.UnitsMode.Pixel;
                ...
                string saveString =     imgLstIndex.Text               + "," 
                    ...
                    if (!batchFirst)
                    {
                        ...
                        IniFile AppliIni = new IniFile(INIpath);
                        AppliIni.WriteValue("Batch roi", "First Done", batchFirst.ToString());
                }
                    else
                    {
        ...
        private Rect GetDeticatedRoiFromList(int ImageIndex)
        {
            Rect rect = new Rect();
            ...
            else if (ImageIndex > 9 && ImageIndex < 100)
            {
                ...
                //    ComboBox list01 = new ComboBox();
                //    list01.Items.AddRange(result01.ToArray());

                //    int iSize = 400;
                //    string selectedROI = " ";
                //    ContextMenuStrip contextMenuStrip13 = new ContextMenuStrip();
                //    DialogResult dr1 = ShowInputDialog(ref selectedROI, list01, "Select roi to use", Color.GreenYellow, iSize, ref contextMenuStrip13);
            ...
            //select from multi roi 
            //List<string> result01 = lst.FindAll(s => s.Substring(0, 3) == (ImageIndex.ToString() + ","));
            if (result01.Count > 1)
            {
                //display input dialog for selection of VP to copy as
                ComboBox list01 = new ComboBox();
        ...
        private void chkUsePPevaluationROI_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (chkUsePPevaluationROI.Checked && !xNoAction)
                ApplyROI(false, ImageDimensions, false, rect);
        }
        ...
        private static DialogResult ShowInputDialog(ref string input, ComboBox cmbList, string sHeader, Color color, int iSize, ref ContextMenuStrip cms, bool xNoCancelButton)
        {
            //System.Drawing.Size size = new System.Drawing.Size(400, 240);  //original = 200, 70
            System.Drawing.Size size = new System.Drawing.Size(iSize, 240);  //original = 200, 70

            Form inputBox = new Form();

            //inputBox.ContextMenuStrip = cms;
